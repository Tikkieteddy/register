...
/**
 * ระยะเวลารอก่อน retry แต่ละครั้ง — 2 วิ, 5 วิ, 15 วิ (รวมรอไม่เกิน 22 วินาที)
 *
 * ⚠️ ห้ามยืดให้ยาวกว่านี้ และนี่คือเหตุผล
 *
 *    เดิมตั้งไว้ 2 + 10 + 60 = 72 วินาที ซึ่ง "รันไม่จบ" บน Vercel
 *    เพราะฟังก์ชันบนแพ็กเกจ Hobby ถูกตัดที่ 60 วินาที งานส่งอีเมลจึงถูกหั่นกลางคัน
 *    แถวใน email_logs ค้างสถานะ queued ตลอดกาล ไม่กลายเป็น failed
 *    แล้วปุ่ม "ส่งซ้ำทุกฉบับที่ค้าง" ก็มองไม่เห็นมัน = อีเมลหายไปเงียบ ๆ
 *
 *    และหัวข้อ 8.2 กำหนดว่าอีเมลต้องถึงภายใน 30 วินาที การรอ retry 60 วินาที
 *    จึงขัดกับข้อกำหนดอยู่แล้วตั้งแต่ต้น
 *
 *    ถ้าครบ 4 ครั้งแล้วยังไม่สำเร็จ แปลว่าเป็นปัญหาที่ต้องแก้ที่ต้นทาง (โควตาเต็ม
 *    โดเมนถูกบล็อก คีย์ผิด) ไม่ใช่ปัญหาชั่วคราวที่รอนานขึ้นแล้วจะหาย
 *    ให้ไปจบที่ปุ่มส่งซ้ำในหน้า Admin แทน
 */
