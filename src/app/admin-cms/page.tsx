...
export default async function AdminDashboardPage({
  ...
  if (!event) {
    return (
      <p className="text-sm text-muted">
        ...
        <Link href="/admin-cms/settings" className="text-primary-dark underline">
          ตั้งค่างาน
        </Link>
      </p>
    );
  ...
  const regBase = "/admin-cms/registrations";
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
        ...
        <KpiCard label="อีเมลส่งไม่สำเร็จ" value={data.kpi.failedEmails} unit="ฉบับ" tone="alert" href="/admin-cms/emails?status=failed" hint={data.kpi.failedEmails > 0 ? "ต้องกดส่งซ้ำ" : undefined} />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        ...
        <ChartFrame no={8} title="ผลงานรายลิงก์" question="ลิงก์ไหนคุ้มที่สุด — คลิกเยอะไม่ได้แปลว่าลงทะเบียนเยอะ" footer={<Link href="/admin-cms/links" className="text-primary-dark underline">จัดการลิงก์ติดตามผลทั้งหมด</Link>}>
          {data.links.length > 0 ? (
